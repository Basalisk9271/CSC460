...
int main(int argc, char** argv) {
    ...
    switch(myId) { 
        // Trigger the synchronizer functionality
        case(SYNCHRONIZER): { 
            int tagger = 0;
            while(!END) {
                p(0, full);
        ...
        // Trigger the manager functionality   
        case(MANAGER): {
        ...
        // Trigger the Disply functionality
        case(DISPLAYER): {
...
// Initialize necessary shared resources, semaphores, and set up simulated RAM
// Also write the file information needed for the other producer and shutdown commands
void initResources(char** argv) {
    ...
    // Initialize semaphores for Mutex, Full, and Empty
    ...
    // Trigger the manager to write the information for the resource IDs
    FILE *fp;
...
// Clear a completed proc from simulated RAM
...
// Displays the list of Processes in the Queue
void displayProcList() {
    int i;
    printf("\nID thePID Size Sec\n");
    for(i=0; i<26; i++) {
        if(q[i].pid != -1 && q[i].time>0) // don't show if time requirement is met
            printf("%c. %6d %4d %3d\n", q[i].tag, q[i].pid, q[i].blocks, q[i].time);
    }
    printf("\n");
}
